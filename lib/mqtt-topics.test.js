...
describe("topicList", () => {
  it("includes Teslamate stats and location topics", () => {
    ...
    assert.ok(topics.includes("teslamate/cars/1/tpms_soft_warning_fl"));
    assert.equal(parseTopic("teslamate/cars/1/service_mode").key, "serviceMode");
    assert.ok(topics.includes("teslamate/cars/1/service_mode"));
    assert.ok(topics.includes("teslamate/cars/1/geofence"));
    assert.ok(topics.includes("teslamate/cars/1/locked"));
    assert.ok(topics.includes("teslamate/cars/1/plugged_in"));
  });
});

describe("subscribeTopic", () => {
  it("subscribes to the car prefix wildcard so retained values arrive on connect", () => {
    assert.equal(subscribeTopic({ carID: "2" }), "teslamate/cars/2/#");
    assert.equal(subscribeTopic({ mqttTopic: "teslamate/cars/1" }), "teslamate/cars/1/#");
  });
});
